...
function SettingsDrawer({ onDrawerToggle, open }: SettingsDrawerProps) {
  const {
    changeCollapsed,
    changeDirection,
    changeMode,
  ...
  const handleDirectionChange = (_: any, directionMode: 'ltr' | 'rtl') => {
    changeDirection(directionMode);
  };
  ...
  const handleModeChange = (_: any, modeFlag: string) => {
    changeMode(modeFlag);
  };

  const handleSidebarChange = (_: any, collapsedMode: boolean) => {
    changeCollapsed(collapsedMode);
  };
